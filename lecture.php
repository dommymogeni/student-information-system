...
$totcourse = mysqli_query($conn, "SELECT * FROM lectures");
...
if(isset($_POST["button"]))
{
  ...
  $sql="INSERT INTO lectures (lecid, password, courseid, lecname, gender, address ,contactno)VALUES('$_POST[lecid]','$pwde', '$_POST[course]',  '$_POST[lecname]', '$_POST[gender]','$_POST[address]','$_POST[contactno]')";
...
{
  $result = mysqli_query($conn, "SELECT * FROM lectures where lecid='$_GET[slid]'");	
  while($row1 = mysqli_fetch_array($result))
  {
	  $totid = $row1["lecid"];
	  $lecname = $row1["lecname"];
	  $password = $row1["l_password"];
...
<form name="form1" method="post" action="" id="formID">
  ...
  <p>
    <input type="submit" name="button" id="button2" value="Insert" />
