...
<section id="page">
  ...
  <section id="contents">
    ...
    <article class="post">
      <header class="postheader">
        <h2>Search by Name or course id</h2>
      </header>
      ...
      <form name="form2" method="post" action="">
        ...
        <p>
          ...
          <?php
            $rescourse = mysqli_query($conn, "SELECT * FROM course");
          ?>
          
          <select name="select" id="select">          
            <?php
              while($row1 = $rescourse ->fetch_assoc()){
                echo "<option>". $row1['courseid']."</option>";
              }
            ?>
          </select>
        </p>
...
<?php
  if(isset($_POST['submitresult'])){
    $searchstu = mysqli_query($conn, "SELECT * FROM studentdetails where (studfname LIKE '$_POST[textfield2]' OR `studlname` LIKE '$_POST[textfield2]') AND courseid ='$_POST[select]' ");
    ?>
